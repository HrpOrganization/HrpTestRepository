...
public class WindowFormElementServiceImpl implements WindowFormElementService {
	 ...
	 * @param writeSometing
	 *            目标填写值
	 * @param originTargetFieldCodes
	 *            一组或多组查询的参数值
	 * @throws Exception
	 */
	@Override
	public void guideFieldWrite(WebDriver driver, int windowLevel, String windowName, String targetFieldName,
			String writeSometing, String... originTargetFieldCodes) throws Exception {
		...
		if (originTargetFieldCodes.length > 0) {
			...
			keyValue = originTargets.get(0).substring(originTargets.get(0).indexOf(",") + 1,
					originTargets.get(0).length());
			if (windowLevel == 1) {
			...
			if (targetPaths.size() > 0) {
				for (WebElement targetPath : targetPaths) {
			...
			while (originTargets.size() > 0) {
				...
				keyValue = originTargets.get(0).substring(originTargets.get(0).indexOf(",") + 1,
						originTargets.get(0).length());
				if (windowLevel == 1) {
					targetOriginHeadClassPath = "//span[contains(@class,'header-text') and contains(text(),'"
							+ windowName + "')]" + "/../.." + "//div[contains(@class,'x-grid3-header')]"
							+ "//*[contains(@class,'x-component') and contains(text(),'" + keyName + "')]" + "/../..";
				}

				// 获取目标CLASS名(获取CLASS下的需求名)
				targetOriginClassCode = ElementTools.getFormListClassItemName(driver,
						By.xpath(targetOriginHeadClassPath));
				targetOriginPath = "//div[contains(@class,'" + targetOriginClassCode + "') and contains(text(),'"
						+ keyValue + "')]" + "/../../../../..";
				targetSonPaths = driver.findElements(By.xpath(targetOriginPath));
				for (WebElement targetSonPath : targetSonPaths) {
					targetSonIds.add(targetSonPath.getAttribute("id"));
				}
				originTargets.remove(0);
				List<String> result = new ArrayList<>();
				for (String id : targetSonIds) {
		...
		/* String */
		String targetClassCode = ElementTools.getFormListClassItemName(driver, By.xpath(targetHeadClassPath));
		targetRowClassPath = "//*[@id='" + targetIdStore + "']" + "//td[contains(@class,'" + targetClassCode
				+ "')]/div";
