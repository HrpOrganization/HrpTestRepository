...
public class ADebitNotePatent {
	...
	public Object[][] getExcelData() throws IOException, BiffException {
		...
		sheetName = "ADebitNotePatent";
		Object[][] excelData = ExcelOperation.getExcelData(fileNamePath, sheetName, keyField);
		return excelData;
	}

	@Test(dataProvider = "excelData")
	public void aDebitNotePatent(HashMap<String, String> excelData) throws Exception {
		// 进入专利入库 → 应付款通知单(专利)
		LoginMethodServiceImpl loginMethodService = new LoginMethodServiceImpl();
		loginMethodService.loginPage(driver, "专利入库", "应付款通知单（专利）");
		// 新增应付款通知单
		...
		desktopFormListOperationService.formListClickTarget(driver, "资产编号", "1112");
		// 生成付款单
