...
public class ADebitNotePaper {
	...
	public Object[][] getExcelData() throws IOException, BiffException {
		...
		sheetName = "ADebitNotePaper";
		Object[][] excelData = ExcelOperation.getExcelData(fileNamePath, sheetName, keyField);
		return excelData;
	}

	@Test(dataProvider = "excelData")
	public void aDebitNotePaper(HashMap<String, String> excelData) throws Exception {
		// 进入论文入库 → 新增论文
		LoginMethodServiceImpl loginMethodService = new LoginMethodServiceImpl();
		loginMethodService.loginPage(driver, "论文入库", "应付款通知单（论文）");
		// 新增应付款通知单
		...
		desktopFormListOperationService.formListClickTarget(driver, "资产编号", "");
		// 生成付款单
