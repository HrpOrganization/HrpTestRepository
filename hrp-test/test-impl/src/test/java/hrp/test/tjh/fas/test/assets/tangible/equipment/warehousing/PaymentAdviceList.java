...
public class PaymentAdviceList {
    ...
    public void paymentAdviceList(HashMap<String, String> excelData) throws Exception {
        //进入设备入库 → 应付款通知单(设备)
        ...
        //填写备注
        windowInputBoxElementService.nullFieldWrite(driver, 1,"新增付款单",
                "width: 1261px; height: 56px", "填写的备注");
        //生成付款单
        ...
        windowDropdownElementService.listFieldWriteSearch
                (driver,1, "收款单位信息确认", "收款单位", supplier, supplier);
        ...
        windowDropdownElementService.listFieldWriteSearch
                (driver,1, "收款单位信息确认", "开户银行", bankName, bankName);
        windowButtunElementService.clickButton(driver,1, "收款单位信息确认", "确认");
        //验证单据是否生成
        String contrastTarget = desktopFormListOperationService.formListContrastTarget
                (driver, "收款单位", supplier, "供应商");
