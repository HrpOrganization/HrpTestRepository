package hrp.test.tjh.fas.test.assetsManagement.vehicle;
...
//无偿捐出（车辆）
public class FreeChargeDonateVehicle {
	...
	public Object[][] getExcelData() throws IOException, BiffException {
		...
		sheetName = "FreeChargeDonateVehicle";
		Object[][] excelData = ExcelOperation.getExcelData(fileNamePath, sheetName, keyField);
		return excelData;
	}

	@Test(dataProvider = "excelData")
	public void freeChargeDonateVehicle(HashMap<String, String> excelData) throws Exception {
		// 进入车辆处置 → 无偿捐出（车辆)
		LoginMethodServiceImpl loginMethodService = new LoginMethodServiceImpl();
		loginMethodService.loginPage(driver, "车辆处置", "无偿捐出（车辆）");
		// 查询
		// 申请资产单元
