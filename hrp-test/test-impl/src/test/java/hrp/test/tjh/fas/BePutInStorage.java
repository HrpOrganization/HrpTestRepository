...
public class BePutInStorage {
	...
	public void bePutInStorage(HashMap<String, String> excelData) throws Exception {
		...
		windowInputBoxElementService.fieldWriteEnter(driver, 1, "核实表", "固定资产卡号", assetCardId);
		// 输入存放地点
		String storageLocation = excelData.get("存放地点");
		windowInputBoxElementService.fieldWrite(driver, 1, "核实表", "存放地点", storageLocation);
		// 输入发票号
		...
		windowDropdownElementService.listFieldWriteSearch(driver, 1, "核实表", "供应商", supplier, supplier);
		// 输入采购员
		String purchasePerson = excelData.get("采购员");
		windowInputBoxElementService.fieldWriteEnter(driver, 1, "核实表", "采购员", purchasePerson);
		// 输入购置日期
		String purchaseDate = excelData.get("购置日期");
		windowInputBoxElementService.fieldWrite(driver, 1, "核实表", "购置日期", purchaseDate);
		// 输入采购计划号
		...
		windowInputBoxElementService.fieldWriteEnter(driver, 1, "核实表", "采购计划号", purchaseNo);
		// WindowButtonElementServiceImpl windowButtonElementService = new
		// WindowButtonElementServiceImpl();
		...
		windowInputBoxElementService.fieldWrite(driver, 1, "核实表详情", "设备单价", assetsValue);
		// 选择国标码
		...
		windowInputBoxElementService.fieldWrite(driver, 1, "核实表详情", "生产厂家", factory);
		// 输入国别
		...
		windowDropdownElementService.listFieldSelect(driver, 1, "核实表详情", "设备成套属性", EquSetType);
		// 输入成套设备名
		// String EquSetName = excelData.get("成套设备名");
		// windowInputBoxElementServiceImplFas.fieldWrite(driver,1, "核实表详情", "成套设备名",
		// EquSetName);
		// 选择设备分类
		String EquTypeCode = excelData.get("设备分类");
		windowDropdownElementService.listFieldSelect(driver, 1, "核实表详情", "设备分类", EquTypeCode);
		// 选择华科共有
		...
		String contrastTarget = desktopFormListOperationService.formListContrastTarget(driver, "设备名称", assetsName,
				"金额");
		...
		// //判断入库单设备是否正确
		// desktopFormListOperationService.formListClickTarget(driver, "发票号",
		// invoiceNo);
		...
		// 判断预算是否执行
		targetMoney = ServerBillContrast.bcsSeverBillContrast(driver, budgetCode, "业务单据号", documentNo, "变动金额(元)");
		Assert.assertEquals(money, targetMoney);
