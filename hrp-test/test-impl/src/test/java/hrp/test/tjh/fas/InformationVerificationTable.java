...
public class InformationVerificationTable {
	...
	public void informationVerificationTable(HashMap<String, String> excelData) throws Exception {
		...
		// windowInputBoxElementService.fieldWrite(driver,1, "核实表详情", "成套设备名",
		// EquSetName);
		// 选择设备分类
		...
		String contrastTarget = desktopFormListOperationService.formListContrastTarget(driver, "发票号", invoiceNo, "供应商");
		Assert.assertEquals(supplier, contrastTarget);
		// 选择生成的单据
		desktopFormListOperationService.formListClickTarget(driver, "发票号", invoiceNo);
		// 对比单据内的设备明细是否正确
		// 获取设备名对应的单价
		contrastTarget = desktopFormListOperationService.formListContrastTarget(driver, "设备名称", assetsName, "单价(元)");
